...
local function toggleESP(novoEstado)
    ESPAtivo = novoEstado
...
-- Configura os eventos para novos jogadores
local function noNovoJogador(jogador)
    if ESPAtivo then
        criarESP(jogador)
    end
...
Players.PlayerAdded:Connect(noNovoJogador)

-- Aplica aos jogadores já no jogo
for _, jogador in ipairs(Players:GetPlayers()) do
    if jogador ~= LocalPlayer then
        noNovoJogador(jogador)
    end
end
...
InspectTab:AddButton({
    ...
    Callback = function()
        inspectBringF()
        OrionLib:MakeNotification({
            Name = "Object/Player Brought",
            Content = "Inspected object/player was brought to your position",
            Time = 3
        })
    end
})

-- Keybind for inspection with toggle
InspectTab:AddBind({
    Name = "Inspection Keybind (Toggle)",
    ...
    Callback = function()
        ...
        if inspectT then
            inspectF()
            OrionLib:MakeNotification({
                Name = "Inspection Activated!",
                Content = "Inspection mode activated - Point at objects",
                Time = 3
            })
        else
            clearInspection()
            OrionLib:MakeNotification({
                Name = "Inspection Deactivated!",
                Content = "Inspection mode deactivated",
                Time = 3
            })
        end
    end
...
InspectTab:AddBind({
    ...
    Callback = function()
        clearInspection()
        OrionLib:MakeNotification({
            Name = "Inspection Cleared",
            Content = "Inspected object was unmarked",
            Time = 2
        })
    end
})

-- NEW SECTION: PLAYER SELECTION
InspectTab:AddSection({
...
local function updatePlayerList()
    ...
    table.insert(players, 1, "None")
    playerDropdown:Refresh(players, true)
end

-- Button to update player list
InspectTab:AddButton({
...
-- Button to bring player using inspectBringF
InspectTab:AddButton({
    Name = "Bring Player",
    Callback = function()
        if selectedPlayer then
            -- Simulate player inspection to use inspectBringF function
...
-- Toggle for bring player loop using inspectBringF
InspectTab:AddToggle({
    Name = "Loop Bring Player (InspectBring)",
    Default = false,
    Callback = function(Value)
        bringLoop = Value
        if Value then
                        ...
                        model:GetPropertyChangedSignal("Parent"):Connect(function()
                            if not model.Parent then
                                if UserInputService:GetLastInputType() == Enum.UserInputType.MouseButton2 then
                                    ...
                                    velocityObj.Velocity = workspace.CurrentCamera.CFrame.LookVector * _G.gravitygrabforce
...
MiscTab:AddParagraph("Instruction:", "After something is without gravity it only comes back to normal when someone else clicks on it")

MiscTab:AddLabel("Some items may Not Work")

-- Credits Tab
local CrTab = Window:MakeTab({
    Name = "Credits",
    Icon = "rbxassetid://18518485889",
    PremiumOnly = false
})
--CreditsTab:AddParagraph("Tip", "Hold to reload bombs")
    CrTab:AddParagraph("👑 owners","make the final decisions and keep the script alive")
    CrTab:AddPlayerParagraph(9584241219)
    CrTab:AddParagraph("🛠 Developers","create features & make the library")
    CrTab:AddPlayerParagraph(1137443778)
    CrTab:AddPlayerParagraph(90063030)
    CrTab:AddParagraph("🥇 honorable mention","people who helped with the script with ideas, inspirations and other things...")
    CrTab:AddPlayerParagraph(7667828277)
    CrTab:AddParagraph("🥈 Testers","people who tested the script to improve the user experience")
    CrTab:AddPlayerParagraph(7197612236)
    local services = {
...
-- Automatically update list when players join/leave
Players.PlayerAdded:Connect(function(player)
    if player ~= Players.LocalPlayer then
        InspectTab:RefreshDropdown("Select Player", GetPlayerList(), true)
    end
end)
